...
test.describe('Auth', () => {
  ...
  test('login as terminal role redirects to terminal page', async ({ page }) => {
    await login(page, `${TENANT_A}-terminal`)
    await expect(page).toHaveURL(/\/tenant\/.+\/terminal/)
  })

  test('login as gate role redirects to gate page', async ({ page }) => {
    await login(page, `${TENANT_A}-gate`)
    await expect(page).toHaveURL(/\/tenant\/.+\/gate/)
  })

  test('login as station role redirects to station page', async ({ page }) => {
...
test.describe('Tenant isolation', () => {
  test('Tenant A admin cannot see Tenant B data', async ({ page }) => {
    await login(page, `${TENANT_A}-admin`)
  ...
  test('Tenant B login reaches Tenant B routes only', async ({ page }) => {
...
test.describe('Admin dashboard', () => {
  ...
  test('shows card list with seeded cards', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /admin/i })).toBeVisible()
    // Seeded 10 cards per tenant
    await expect(page.locator('[data-testid="card-row"], .card-row').first()).toBeVisible({ timeout: 5000 })
  })
...
test.describe('Station — card management', () => {
  ...
  test('shows card list', async ({ page }) => {
    await expect(page.getByText(/Daftar Kartu/i)).toBeVisible()
  })

  test('register new card flow', async ({ page }) => {
  ...
  test('cannot register duplicate card ID', async ({ page }) => {
    // Register first time
    await page.getByText(/Daftarkan Kartu/i).click()
  ...
  test('block card action', async ({ page }) => {
...
test.describe('Scout — balance check (no NFC on desktop)', () => {
  test('scout page shows scan button', async ({ page }) => {
