...
import {
  ...
  type CardPayload,
  type SessionGrant,
} from "#/presentation/hooks/types";
import { trackError } from "#/presentation/hooks/useErrorTracker";
import { localDb, type Card } from "#/presentation/hooks/useLocalDb";
import { cardRepo, onlineStatus, uidRemoteValidator } from "#/presentation/hooks/useRepositories";
import { syncPull } from "#/presentation/hooks/useSyncPull";
...
export function generateCardId(): Uint8Array {
...
export function toPayloadCardId(cardId: string): Uint8Array {
...
export function toPayloadStatus(status: Card["status"]): CardStatus {
  switch (status) {
    case "active":
...
export class CardNotBlankError extends Error {
...
export type IssuancePhase = "idle" | "scanning" | "writing" | "done" | "error";

export interface IssuanceRefs {
...
export interface IssuanceSetters {
 ...
 */
export async function validateUIDForIssuance(
 ...
 */
export async function checkLocalCardConflict(
  capturedSerial: string,
  tenantId: string,
): Promise<void> {
  const existing = await localDb.cards.get([tenantId, capturedSerial]);
 ...
 */
export async function validateCardForRecovery(
