...
function NfcTestPage() {
  ...
  const handleScan = useCallback(async () => {
    if (!supported) { addLog('❌ NDEFReader not available'); return }
    ...
    reader.addEventListener('reading', (event: NDEFReadingEvent) => {
      addLog(`✅ Card detected  serial=${event.serialNumber || '(none)'}  records=${event.message.records.length}`)
      event.message.records.forEach((rec, i) => {
        const bytes = rec.data
          ...
          : new Uint8Array(0)
        addLog(`   [${i}] type="${rec.recordType}"  ${bytes.length} bytes`, `       hex: ${toHex(bytes)}`)
      })
    })
  ...
  return (
    <div className="min-h-screen bg-background p-4 max-w-lg mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Direct NDEFReader API — no payload encoding. Dev/LAN only.</p>
      </div>
      ...
      <section className="space-y-2">
        <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Read</h2>
        ...
        <p className="text-xs text-muted-foreground">Shows raw NDEF records — works on formatted cards only.</p>
      </section>
      ...
      {/* Write */}
      <section className="space-y-2">
        <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Write (tap card when prompted)</h2>
        ...
        <Button variant="destructive" size="sm" disabled={!supported} onClick={handleClear} className="w-full">
          Format / Clear Tag
        </Button>
        <p className="text-xs text-muted-foreground">
          <strong>Write Text</strong> → NDEF text record (proves basic NDEF write works).<br />
          <strong>Write 32 Raw Bytes</strong> → <code>unknown</code> record, same type the app uses.<br />
      ...
      <section className="space-y-3">
        <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Write Payload (tap card when prompted)</h2>
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1">
            <Label className="text-xs">Name</Label>
            <Input value={payloadName} onChange={(e) => setPayloadName(e.target.value)} placeholder="Test User" />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">User ID</Label>
            <Input value={payloadUserId} onChange={(e) => setPayloadUserId(e.target.value)} placeholder="1" />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">Balance (IDR)</Label>
            <Input value={payloadBalance} onChange={(e) => setPayloadBalance(e.target.value)} placeholder="50000" />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">Expires (days)</Label>
            <Input value={payloadExpiresOffset} onChange={(e) => setPayloadExpiresOffset(e.target.value)} placeholder="365" />
          </div>
        </div>
        ...
        <p className="text-xs text-muted-foreground">
          Encodes a full <code>CardPayload</code> via <code>makeFreshCard</code> and writes it as an <code>unknown</code> record.
        </p>
      </section>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Log</h2>
          <Button variant="ghost" size="sm" onClick={handleClearLog} className="text-xs h-6">Clear</Button>
        </div>
