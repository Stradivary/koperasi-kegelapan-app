      ...
      POST: async ({ request }) => {
        ...
        for (const event of body.events as ReconcileEvent[]) {
          try {
            ...
            if (!card) { rejected.push({ id: event.cardId, reason: 'Card not found' }); continue }
            if (event.counter <= card.counter) { rejected.push({ id: event.cardId, reason: 'Stale counter' }); continue }
