...
function makePayload(
  ...
  return {
    ...
    logEntries: [],
    trailer: { expiresAt: 2000000000, keyVersion: 1, rootHash: new Uint8Array(6), counterBind: 5, hmac: new Uint8Array(8), activePtr: 0 },
  }
}
...
describe('validateTransition', () => {
  ...
  it('rejects write on blocked card', () => {
    const r = validateTransition(makePayload(CardState.IDLE, CardStatus.BLOCKED_TAMPER), 'gate_checkin', NOW)
    expect(r.valid).toBe(false)
  })

  it('allows force_checkout on expired session', () => {
    const staleTime = NOW - 30 * 60 * 60
    const r = validateTransition(makePayload(CardState.CHECKED_IN, CardStatus.ACTIVE, staleTime), 'force_checkout', NOW)
    expect(r.valid).toBe(true)
    expect(r.nextState).toBe(CardState.CHECKED_OUT)
  })
})
...
describe('isWriteEligible', () => {
  ...
  it('rejects expired grant', () => {
    const r = isWriteEligible(makePayload(CardState.CHECKED_IN), makeGrant(NOW - 100), 'debit', NOW)
  ...
  it('rejects blocked card', () => {
    const r = isWriteEligible(makePayload(CardState.CHECKED_IN, CardStatus.BLOCKED_FRAUD), makeGrant(), 'debit', NOW)
    expect(r.eligible).toBe(false)
  })
})
...
describe('isSessionExpired', () => {
  it('not expired within 24h + 1h drift', () => {
    expect(isSessionExpired(makePayload(CardState.CHECKED_IN, CardStatus.ACTIVE, NOW - 20 * 3600), NOW)).toBe(false)
  })

  it('expired after 25h', () => {
    expect(isSessionExpired(makePayload(CardState.CHECKED_IN, CardStatus.ACTIVE, NOW - 26 * 3600), NOW)).toBe(true)
  })
