...
vi.mock("../../db/local-db", () => {
  ...
  return {
    localDb: {
      transactionLog,
      // Support localDb.transaction("rw", table, callback) by just executing the callback
...
describe("transactionLogService", () => {
  ...
  describe("recordTransaction", () => {
    ...
    it("rejects on IndexedDB write failure", async () => {
      const table = getMockedTable();
      const mockFirst = vi.fn().mockResolvedValue(undefined);
      table.where.mockReturnValue({
        equals: vi.fn().mockReturnValue({ first: mockFirst }),
      ...
      table.add.mockRejectedValueOnce(new Error("QuotaExceededError"));

      const entry = makeEntry();

      await expect(recordTransaction(entry)).rejects.toThrow("QuotaExceededError");
      expect(table.add).toHaveBeenCalledTimes(1);
    });

    it("throws DuplicateTransactionError on ConstraintError", async () => {
      const table = getMockedTable();
      const mockFirst = vi.fn().mockResolvedValue(undefined);
      table.where.mockReturnValue({
        equals: vi.fn().mockReturnValue({ first: mockFirst }),
      ...
      // ConstraintError propagates as-is (caught by recordCardWrite's try/catch)
      await expect(recordTransaction(entry)).rejects.toThrow();
      expect(table.add).toHaveBeenCalledTimes(1);
    });
  });
