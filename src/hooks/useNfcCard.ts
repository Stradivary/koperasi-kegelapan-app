    ...
    reader.addEventListener('reading', async (event: NDEFReadingEvent) => {
      ...
      if (phase === 'scanning') {
        ...
        if (!raw) {
          ...
          setState((s) => ({ ...s, phase: 'error', error: 'Kartu tidak berisi data yang valid', tamperDetected: false }))
          return
        }
        ...
        try {
          ...
          setState({ phase: 'ready', payload, serialNumber: event.serialNumber, error: null, tamperDetected: false })
        } catch (e) {
          if (signal.aborted) return
          phaseRef.current = 'error'
          setState((s) => ({ ...s, phase: 'error', error: `Decode gagal: ${e}`, tamperDetected: true }))
        }
        return
      }
      ...
      if (phase === 'writing') {
        ...
        try {
          ...
          const resultPayload = decodePayload(raw)
          phaseRef.current = 'success'
          setState({ phase: 'success', payload: resultPayload, serialNumber, error: null, tamperDetected: false })
        } catch (e) {
  ...
  const write = useCallback(
    async (updatedPayload: CardPayload): Promise<boolean> => {
      ...
      try {
        ...
        const raw = await prepareWrite(currentPayload, updatedPayload, grant)
        pendingWriteRef.current = { raw, currentPayload, updatedPayload, serialNumber: currentSerial }
        return true
      } catch (e) {
        phaseRef.current = 'error'
