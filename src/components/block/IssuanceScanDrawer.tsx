          ...
          >
            <div
              className={[
                ...
                i < active ? 'bg-primary text-primary-foreground' : i === active ? 'bg-primary text-primary-foreground ring-2 ring-primary/30' : 'bg-muted text-muted-foreground',
              ].join(' ')}
            >
              {i < active ? '✓' : i + 1}
            </div>
            <span className={['text-[9px] whitespace-nowrap', i === active ? 'text-primary font-semibold' : 'text-muted-foreground'].join(' ')}>
              {label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div className={['h-0.5 w-8 shrink-0 mb-3', i < active ? 'bg-primary' : 'bg-muted'].join(' ')} />
          )}
        </div>
      ))}
    </div>
...
function NfcPulse({ color = 'primary' }: { color?: 'primary' | 'warning' }) {
  ...
  const bg = color === 'warning' ? 'bg-signal-bg-warning border-signal-warning' : 'bg-primary/10 border-primary'
  ...
  return (
    <div className="relative flex items-center justify-center w-40 h-40">
      ...
      <span className={`absolute inset-4 rounded-full border-2 ${ring}/30 animate-ping [animation-delay:300ms]`} />
      <span className={`absolute inset-8 rounded-full border-2 ${ring}/40 animate-ping [animation-delay:600ms]`} />
      <span className={`relative z-10 w-24 h-24 rounded-full ${bg} border-2 flex items-center justify-center`}>
      ...
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>
            ...
            {isDone && (mode === 'write' ? 'Kartu Berhasil Ditulis' : (payload?.identity.name ?? 'Kartu Dibaca'))}
            {isError && 'Gagal'}
          </DrawerTitle>
          <DrawerDescription>
        ...
        <div className="px-4 overflow-y-auto max-h-[60vh]">
          ...
          {isScanning && (
            <div className="flex flex-col items-center justify-center py-8 gap-6">
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Dekatkan kartu ke bagian belakang perangkat</p>
              </div>
            </div>
          )}
          ...
          {isWriting && (
            <div className="flex flex-col items-center justify-center py-8 gap-6">
              ...
              <div className="text-center space-y-1">
                <p className="text-sm font-medium text-signal-warning">Tap & tahan kartu ke perangkat</p>
                <p className="text-xs text-muted-foreground">Jangan pindahkan kartu sampai proses selesai</p>
              </div>
            </div>
          )}
