        ...
        <div key={label} className="flex items-center gap-1">
          <div
            className={[
              ...
              i < active ? 'bg-brand text-white' : i === active ? 'bg-brand text-white ring-2 ring-brand/30' : 'bg-muted text-muted-foreground',
            ].join(' ')}
          >
            {i < active ? '✓' : i + 1}
          </div>
          {i < STEPS.length - 1 && (
            <div className={['h-0.5 w-6 shrink-0', i < active ? 'bg-brand' : 'bg-muted'].join(' ')} />
          )}
        </div>
      ))}
    </div>
    ...
    <Drawer open={open} onOpenChange={onOpenChange} direction="bottom">
      <DrawerContent>
        <DrawerHeader>
          ...
          {hasCard && payload && (
            <DrawerDescription asChild>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className={isCheckedIn ? 'text-xs font-medium text-signal-valid' : 'text-xs font-medium text-signal-info'}>
          ...
          {isWriting && (
            <DrawerDescription>Tap kartu lagi ke perangkat dan tahan sampai selesai</DrawerDescription>
          )}
        </DrawerHeader>
        ...
        <DrawerFooter>
          ...
          {isError && (
            <>
              {!tamperDetected && (
                <Button onClick={onRetry} className="w-full bg-brand-dark hover:bg-brand-dark/90 text-white">
                  Coba Lagi
                </Button>
              )}
              <Button variant="outline" onClick={onClose} className="w-full">
