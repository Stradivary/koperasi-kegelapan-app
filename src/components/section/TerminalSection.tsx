...
export function TerminalSection({ tenantId, tenantName, accountId, deviceId, terminalId }: TerminalSectionProps) {
  ...
  async function handleDebit() {
    ...
    if (isNaN(amount) || amount <= 0) { setTxError('Nominal tidak valid'); return }
    if (amount > MAX_TRANSACTION_AMOUNT) {
      setTxError(`Maks Rp ${MAX_TRANSACTION_AMOUNT.toLocaleString('id-ID')}`)
      return
    }
    if (state.payload.wallet.balance < amount) { setTxError('Saldo tidak cukup'); return }
  ...
  return (
    <KioskLayout title="Terminal" tenantName={tenantName} trailing={syncTrailing}>
      <div className="flex-1 flex flex-col items-center justify-center gap-6 p-6">
        ...
        {grantLoading && (
          <p className="type-body2 text-white/70 animate-pulse">Memuat sesi...</p>
        )}
        {!grant && !grantLoading && (
